...
MAX_WRONG_GUESSES = 6
CYRILLIC_ALPHABET = ['А', 'Б', 'В', 'Г', 'Д', 'Е', 'Ё', 'Ж', 'З', 'И', 'Й', 'К', 
                    'Л', 'М', 'Н', 'О', 'П', 'Р', 'С', 'Т', 'У', 'Ф', 
...
class Hangman:
    ...
    def _build_letters_frame_ru(self):
        letter_groups = [
            CYRILLIC_ALPHABET[i : i + 4]
            for i in range(0, len(CYRILLIC_ALPHABET), 4)
        ]
        letter_buttons = [
            [
                sg.Button(
    ...
    def _select_word_ru(self):
        with open("words-ru.txt", mode="r", encoding="utf-8") as words:
            word_list = words.readlines()
        return choice(word_list).strip().upper()
    ...
    def _new_game(self):
        self._target_word = self._select_word_ru()
        self._restart_game()

    """ def _new_game_ru(self):
        self._target_word = self._select_word_ru()
        self._restart_game() """
